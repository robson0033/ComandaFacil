...
const SUBSCRIPTION_PIX_EXPIRATION_MINUTES = 30;
...
async function loadWebhookResource(data) {
  ...
  const mesaPayment = await mesaPixPaymentService.loadWebhookPayment(data.resourceId);
  if (mesaPayment) return mesaPayment;
  ...
  const recoveredMesa = await mesaPixPaymentService.recoverWebhookByExternalReference(platformPayment);
  if (recoveredMesa) return recoveredMesa;
...
async function processWebhookEvent(event, loaded) {
  ...
  try {
    ...
    if (loaded.kind === "mesa_payment") {
      stage = "webhook_mesa_payment_processing";
      return await mesaPixPaymentService.processWebhookPayment(event, loaded.resource, loaded.attempt);
    }
