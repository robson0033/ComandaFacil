    ...
    pedidosAbertos.forEach((pedido) => {
      (pedido.itens || []).forEach((item) => {
        itensPendentesMesa.push({
          ...
          remocaoSolicitacaoStatus: String(item.remocaoSolicitacaoStatus || "nenhuma"),
          remocaoSolicitadaEm: item.remocaoSolicitadaEm || null,
...
exports.solicitarRemocaoPedidoMesa = async (req, res) => {
  try {
    const mesa = await Mesa.findOne({
      token: req.params.token,
      status: { $ne: "inativa" },
    ...
    if (!mongoose.isValidObjectId(pedidoId) || !mongoose.isValidObjectId(itemId)) {
      return res.status(404).json({
        success: false,
        code: "ITEM_PEDIDO_NAO_ENCONTRADO",
        message: "Item não encontrado para esta mesa.",
      });
    }
    ...
    // O operador posicional $ altera somente o item que corresponde ao itemId.
    // O pedido pode conter vários itens enviados juntos, mas os demais permanecem intactos.
    const pedido = await Pedido.findOneAndUpdate(
      ...
      {
        $set: {
          "itens.$.remocaoSolicitacaoStatus": "pendente",
          "itens.$.remocaoSolicitadaEm": new Date(),
          "itens.$.remocaoDecididaEm": null,
          "itens.$.remocaoDecididaPor": null,
        },
      },
      { returnDocument: "after", runValidators: true },
    );
    ...
    if (!pedido) {
      const existente = await Pedido.findOne(filtroBase)
        .select("_id pagamentoStatus status itens._id itens.remocaoSolicitacaoStatus")
        .lean();
      const item = (existente?.itens || []).find(
        candidato => String(candidato?._id || "") === itemId,
      );
      ...
      if (String(item.remocaoSolicitacaoStatus || "") === "pendente") {
        return res.json({
          success: true,
          pedidoId: String(existente._id),
          itemId,
          aguardandoAprovacao: true,
          message: "A solicitação deste item já foi enviada e aguarda autorização do estabelecimento.",
        });
      }

      return res.status(409).json({
        success: false,
        code: "ITEM_NAO_PODE_SOLICITAR_TROCA",
        message: "Este item não pode mais ser alterado porque o pedido já foi pago, cancelado ou finalizado.",
      });
    }
    ...
    return res.json({
      ...
      itemId,
      aguardandoAprovacao: true,
      message: "Solicitação do item enviada. Aguarde a autorização do estabelecimento.",
    });
  } catch (error) {
    appLogger.error("Erro ao solicitar remoção de item da mesa:", error);
    return res.status(500).json({
      success: false,
      code: "ITEM_MESA_SOLICITACAO_REMOCAO_FALHOU",
      message: "Não foi possível enviar a solicitação agora.",
    });
  }
};

function totaisPedidoMesaDepoisDaRemocao(pedido, itens) {
  const subtotalCentavos = itens.reduce((total, item) => (
    total + Math.max(0, Math.round(Number(item?.subtotal || 0) * 100))
  ), 0);
  const custoCentavos = itens.reduce((total, item) => {
    const custoUnitario = Math.max(0, Number(item?.custoUnitarioSnapshot || 0));
    const quantidade = Math.max(0, Number(item?.quantidade || 0));
    return total + Math.round(custoUnitario * quantidade * 100);
  }, 0);
  const taxaEntregaCentavos = Math.max(0, Math.round(Number(pedido?.taxaEntregaCentavos || 0)));
  return {
    subtotalProdutos: subtotalCentavos / 100,
    total: (subtotalCentavos + taxaEntregaCentavos) / 100,
    custo: custoCentavos / 100,
  };
}
...
exports.aprovarRemocaoPedidoMesa = async (req, res) => {
  try {
    ...
    const atual = await Pedido.findOne({
      _id: pedidoId,
      estabelecimentoId: idEstabelecimento,
      canal: "mesa",
      excluido: { $ne: true },
      pagamentoStatus: "pendente",
      status: { $nin: ["cancelado", "finalizado"] },
      itens: { $elemMatch: { _id: itemId, remocaoSolicitacaoStatus: "pendente" } },
    });

    if (!atual) {
      return res.status(409).json({
        success: false,
        code: "SOLICITACAO_ITEM_NAO_ELEGIVEL",
        message: "A solicitação deste item não está mais disponível. O pedido pode ter sido pago ou alterado.",
      });
    }

    const itemRemovido = atual.itens.find(item => String(item?._id || "") === itemId);
    if (!itemRemovido) {
      return res.status(409).json({
        success: false,
        code: "SOLICITACAO_ITEM_NAO_ELEGIVEL",
        message: "O item solicitado não existe mais neste pedido.",
      });
    }

    const itensRestantes = atual.itens
      .filter(item => String(item?._id || "") !== itemId)
      .map(item => item.toObject ? item.toObject() : item);
    const pedidoEncerrado = itensRestantes.length === 0;
    const totais = totaisPedidoMesaDepoisDaRemocao(atual, itensRestantes);
    ...
    if (pedidoEncerrado) {
      ...
      set.pagamentoStatus = "cancelado";
      // Compatibilidade com os fluxos antigos que ocultam pedido totalmente cancelado por troca.
      set.remocaoSolicitacaoStatus = "aprovada";
      set.remocaoDecididaEm = new Date();
      set.remocaoDecididaPor = req.session.user.id;
    }
    ...
    if (Number.isInteger(atual.__v)) filtroAtualizacao.__v = atual.__v;

    const update = { $set: set };
    if (Number.isInteger(atual.__v)) update.$inc = { __v: 1 };
    ...
    if (!pedido) {
      return res.status(409).json({
        success: false,
        code: "SOLICITACAO_ITEM_CONCORRENTE",
        message: "O pedido mudou enquanto a solicitação era analisada. Atualize a comanda e tente novamente.",
      });
    }

    // Jobs ainda não enviados contêm um snapshot antigo com o item removido.
    // Eles são cancelados para não imprimir uma comanda desatualizada. Jobs já enviados
    // não são reenviados automaticamente, evitando duplicidade na cozinha.
    await PrintJob.updateMany(
      ...
      },
      {
        $set: {
          status: "cancelado",
          erro: "Item removido da mesa antes da impressão; snapshot antigo cancelado.",
          lockedBy: "",
          leaseToken: "",
          leaseExpiresAt: null,
          nextAttemptAt: null,
    ...
    const possuiOutrosPedidos = pedidoEncerrado && await Pedido.exists({
      estabelecimentoId: idEstabelecimento,
      mesaId: pedido.mesaId,
      excluido: { $ne: true },
      _id: { $ne: pedido._id },
    ...
    if (pedidoEncerrado && !possuiOutrosPedidos && pedido.mesaId) {
      await Mesa.updateOne(
        { _id: pedido.mesaId, estabelecimentoId: idEstabelecimento },
        { $set: { status: "livre" } },
      );
    ...
    await registrarAuditoria({
      ...
      dadosResumidos: {
        ...
        itemId,
        itemNome: String(itemRemovido.nome || "").slice(0, 120),
        pedidoEncerrado,
        totalRestante: pedido.total,
      },
      operationKey: `auditoria:troca_item_mesa_aprovada:${pedido._id}:${itemId}`,
    });
    ...
    return res.json({
      ...
      mesaId: String(pedido.mesaId || ""),
      pedidoEncerrado,
      message: pedidoEncerrado
        ? "Troca aprovada. O único item foi removido e o pedido saiu da conta da mesa."
        : "Troca aprovada. Somente o item solicitado foi removido da conta da mesa.",
    });
  } catch (error) {
    appLogger.error("Erro ao aprovar troca de item da mesa:", error);
    return res.status(500).json({
      success: false,
      code: "APROVACAO_TROCA_ITEM_MESA_FALHOU",
...
exports.recusarRemocaoPedidoMesa = async (req, res) => {
  try {
    ...
    const pedido = await Pedido.findOneAndUpdate(
      ...
      {
        $set: {
          "itens.$.remocaoSolicitacaoStatus": "recusada",
          "itens.$.remocaoDecididaEm": new Date(),
          "itens.$.remocaoDecididaPor": req.session.user.id,
        },
      },
      { returnDocument: "after", runValidators: true },
    );
    ...
    if (!pedido) {
      const existente = await Pedido.findOne({
        ...
        .select("_id mesaId itens._id itens.remocaoSolicitacaoStatus")
        .lean();
      const item = (existente?.itens || []).find(
        candidato => String(candidato?._id || "") === itemId,
      );

      if (String(item?.remocaoSolicitacaoStatus || "") === "recusada") {
        return res.json({
          success: true,
          pedidoId: String(existente._id),
          itemId,
          ...
          idempotent: true,
          message: "A solicitação deste item já estava recusada. O item continua na mesa.",
        });
      }
      ...
      return res.status(409).json({
        success: false,
        code: "SOLICITACAO_ITEM_NAO_ELEGIVEL",
        message: "A solicitação deste item já foi analisada ou não existe mais.",
      });
    }
    ...
    await registrarAuditoria({
      ...
      operationKey: `auditoria:troca_item_mesa_recusada:${pedido._id}:${itemId}:${Date.now()}`,
    });
    ...
    return res.json({
      ...
      itemId,
      mesaId: String(pedido.mesaId || ""),
      message: "Solicitação recusada. Somente este item continua na mesa.",
    });
  } catch (error) {
    appLogger.error("Erro ao recusar troca de item da mesa:", error);
    return res.status(500).json({
      success: false,
      code: "RECUSA_TROCA_ITEM_MESA_FALHOU",
