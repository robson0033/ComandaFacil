...
async function finalizarMesaAprovada(attempt, payment, { source = "status" } = {}) {
  ...
  const amountNow = totalPedidos(aindaPendentes) / 100;
  if (!idsIguais(currentIds, snapshotIds) || Math.abs(amountNow - Number(attempt.expectedAmount)) > 0.001) {
  ...
  for (const pedido of aindaPendentes) {
    const centavos = Math.max(0, Math.round(Number(pedido.total || 0) * 100));
    pedido.pagamentoStatus = "pago";
    pedido.formaPagamento = "pix_online";
    pedido.pagamentos = [{ formaPagamento: "pix_online", valorCentavos: centavos }];
    ...
    if (!pedido.historicoFinanceiro.some(item => String(item?.operationKey || "") === operationKey)) {
      pedido.historicoFinanceiro.push({
        ...
        status: "pago",
        tipo: "pagamento_mesa_pix",
        statusAnterior: "pendente",
        statusNovo: "pago",
        formaPagamento: "pix_online",
        pagamentos: [{ formaPagamento: "pix_online", valorCentavos: centavos }],
        valor: Number(pedido.total || 0),
        motivo: `Pagamento Pix da mesa confirmado via ${source}.`,
        operationKey,
...
async function gerarPixMesa({ req, estabelecimentoId, mesaId, usuarioId }) {
  ...
  const amount = totalCentavos / 100;
  ...
  if (existing) {
    if (!idsIguais(existing.pedidoIds || [], pedidoIds)
      || Math.abs(Number(existing.expectedAmount || 0) - amount) > 0.001) {
      const error = new Error("A conta da mesa mudou enquanto existe um Pix ativo. Cancele ou concilie o Pix atual antes de continuar.");
    ...
    const payment = await mp("/v1/payments", {
      ...
      body: JSON.stringify({
        ...
        ...(feeConfig.enabled && Number(attempt.platformFeeCents || 0) > 0
          ...
          : {}),
        description: `Conta Mesa ${mesa.numero} - ${String(cfg.nomeEstabelecimento || "ComandaFacil").slice(0, 80)}`,
