      ...
      itens: [
        {
          ...
          // Solicitação de troca/remoção é por ITEM, não pelo pedido inteiro.
          // Isso evita que clicar em um produto marque/remova todos os itens
          // enviados juntos no mesmo pedido da mesa.
...
const printJobSchema = new mongoose.Schema({
  ...
  mesaPaymentAttemptId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: "MesaPaymentAttempt",
    default: null,
    index: true,
  },
  tipo: {
  ...
  motivo: {
    type: String,
    enum: ["order_created", "payment_approved", "manual", "pix_mesa"],
...
const mesaPaymentAttemptSchema = new mongoose.Schema({
  attemptId: { type: String, required: true, immutable: true },
  externalReference: { type: String, required: true, immutable: true },
  estabelecimentoId: { type: mongoose.Schema.Types.ObjectId, required: true, index: true },
  ...
  remoteCancellationStatus: { type: String, default: "", maxlength: 60 },
  reconciliationStatus: { type: String, default: "pending" },
  reconciliationReason: { type: String, default: "", maxlength: 200 },
  pixCopiaCola: { type: String, default: "", maxlength: 5000 },
  pixQrCodeBase64: { type: String, default: "", maxlength: 100000 },
  printError: { type: String, default: "", maxlength: 500 },
  platformFeePercent: { type: Number, default: null },
  platformFeeCents: { type: Number, default: 0, min: 0 },
  platformFeeStatus: {
    type: String,
...
mesaPaymentAttemptSchema.index({ attemptId: 1 }, { unique: true, name: "mesa_payment_attempt_id_unique" });
mesaPaymentAttemptSchema.index({ externalReference: 1 }, { unique: true, name: "mesa_payment_external_reference_unique" });
mesaPaymentAttemptSchema.index({ paymentId: 1 }, {
  unique: true,
  partialFilterExpression: { paymentId: { $type: "string", $gt: "" } },
  name: "mesa_payment_payment_id_unique",
});
mesaPaymentAttemptSchema.index({ idempotencyKey: 1 }, { unique: true, name: "mesa_payment_idempotency_unique" });
mesaPaymentAttemptSchema.index(
  { estabelecimentoId: 1, mesaId: 1 },
  {
    unique: true,
    partialFilterExpression: { ativa: true },
    name: "mesa_payment_active_unique",
  },
);
mesaPaymentAttemptSchema.index({ estabelecimentoId: 1, mesaId: 1, createdAt: -1 }, { name: "mesa_payment_tenant_table" });
const MesaPaymentAttempt = mongoose.model("MesaPaymentAttempt", mesaPaymentAttemptSchema);
